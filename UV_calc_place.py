...
help_user_text = """  Программа UV_lack_calc предназначена для расчета стоимости выполнения выборочной УФ-лакировки. 
...
7. Для проведения расчета нажмите кнопку "Рассчитать". В средней части экрана появятся результаты расчета в соответствующих полях. Если есть 
   ...
   данные (процент заполнения больше 100%, символьные эелементы вместо цифр или др.), после нажатия кнопки "Рассчитать" поле с неправильными
   данными будет кратковременно подсвечено красным цветом и расчёт не будет проведен до момента исправления введённых данных и повтроного
   нажатия кнопки "Рассчитать".
8. Для проведения следующего расчёта можно нажать кнопку "Очистить", что приведёт к удалению всех введённых данных. Программа готова для
   ввода очередных данных.

  Для выбора рабочего профиля или изменения расценок выбираем: "Меню - Расценки".
  В открывшемся окне представлен перечень всех составляющих, необходимых для расчета стоимости лакировки.
...
программы загружается профиль "Основной" 
...
необходимо нажать соответствующую кнопку.
  ...
  Для администрирования созданных профилей выбираем: "Меню - Управление профилями".
  В открывшемся окне отображается путь к месту на диске (локально или по сети) где располагаются файлы с настройками и данными. Далее в окне в 
табличном виде представлены все ранее созданные профили с информацией по ним: Название профиля, Состояние файла с настройками профиля, Дата 
последнего редактирования профиля.
...
В программе используются подключаемые библиотеки:
  ...
  numpy - библиотека для расширения математических вохможностей языка python.
  pdf2image - библиотека для работы с файлами формата *.pdf.
